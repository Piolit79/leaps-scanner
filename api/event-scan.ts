...
import { detectEvents } from './lib/scanner/event-scanner.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  for (let i = 0; i < FALLBACK_LARGE_CAPS.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async (ticker) => {
      try {
        // ~750 calendar days = ~520 trading days for 2-year historical lookback
        ...
        if (!error) found++;
        else console.error(`[${ticker}] DB error: ${error.message}`);
      } catch (e: any) {
